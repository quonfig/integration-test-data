...
| sdk-go                     | `qfg-y8je.6`  | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x]        | [quonfig/sdk-go@885a67b](https://github.com/quonfig/sdk-go/commit/885a67b) (`telemetry_transport_test.go`). Manual clock drives ticks and the per-POST deadline (request context, so `WithHTTPClient` cannot remove it); 5s connect/TLS on the SDK-owned client only. |
| sdk-python                 | `qfg-y8je.7`  | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x]        | [quonfig/sdk-python@c1422f4](https://github.com/quonfig/sdk-python/commit/c1422f4) (`tests/unit/test_telemetry_transport.py`, stdlib `ThreadingHTTPServer` stub, injected clock, `caplog`). `requests` timeouts are real time, so T1 runs compressed (300ms) and asserts the 15000/5000 defaults; `requests` has no whole-request deadline, so 15s bounds each read. Ticks run directly on the test thread. T8 plus an `atexit` final flush (5s, owner-pid guarded against forked children), tested in subprocesses. |
| sdk-ruby                   | `qfg-y8je.8`  | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x]        | [quonfig/sdk-ruby@5c8e208](https://github.com/quonfig/sdk-ruby/commit/5c8e208) (`test/test_telemetry_transport.rb`, WEBrick stub). Net::HTTP deadlines run on the wall clock, so T1/T8 use a compressed timeout and assert the 15000/5000 defaults separately; ticks are fired by the test on a manual clock. |
| sdk-java                   | `qfg-y8je.9`  | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x]        | [quonfig/sdk-java@550d467](https://github.com/quonfig/sdk-java/commit/550d467) (`core/src/test/java/com/quonfig/sdk/telemetry/TelemetryTransportContractTest.java`). JDK `HttpServer` stub, manual `Clock` + `ScheduledExecutorService` (`telemetryClock` / `telemetryScheduler` options), capturing SLF4J logger; no new deps. |
| sdk-net                    | `qfg-y8je.10` | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x]        | [quonfig/sdk-net@a75bec7](https://github.com/quonfig/sdk-net/commit/a75bec7) (`tests/Quonfig.Sdk.Tests/Telemetry/TelemetryTransportContractTests.cs`). T1 pins the timeout-kills-loop fix (`qfg-y8je.1`); `contextUploadMode` default moved to `periodic_example` (sdk-net@ddbe573). Clock seam is an internal `ITelemetryClock` (no `TimeProvider` on netstandard2.0 without a new package); 5s connect via `SocketsHttpHandler.ConnectTimeout` on net8.0 only. |
| sdk-javascript + sdk-react | `qfg-y8je.11` | [x] | [x] | [x] | n/a | n/a | n/a | [x] | beacon [x] | [quonfig/sdk-javascript@43327d8](https://github.com/quonfig/sdk-javascript/commit/43327d8) (`test/telemetry-transport.test.js`). Scripted `fetch` mock (no sockets) + jest fake timers; the 10s deadline is a timer-fired AbortController per request. pagehide sends the live window once via `fetch` keepalive (sendBeacon cannot carry the Authorization header), 2s, retained queue untouched. Regressions pinned: per-request abort timer, network error caught and retryable, `close()` leaves no timer. sdk-react inherits via its pin (suite green against the local build). Retry-After is only honored if api-telemetry exposes it via CORS. |
| sdk-swift                  | `qfg-y8je.12` | n/a | [x] | [x] | n/a | [x] | n/a | n/a | n/a        | [quonfig/sdk-swift@4031081](https://github.com/quonfig/sdk-swift/commit/4031081) (`Tests/QuonfigTests/TelemetryTransportTests.swift`). URLProtocol stub scripted per request, manual clock, capturing `QuonfigLogSink`, the real on-disk queue (one file per batch, deleted only after a 2xx) in a temp dir; ticks called directly. Also covers the 30s floor + Retry-After, a compressed 15s timeout, and the background final flush (~5s `performExpiringActivity`). URLSession has no connect-timeout knob, so the 15s deadline also bounds connect/TLS. |
